//! Linked list borrowed from tokio.
//!
...
impl<L: Link> LinkedList<L, L::Target> {
    ...
    pub(crate) unsafe fn remove(&mut self, node: NonNull<L::Target>) -> Option<L::Handle> {
        if let Some(prev) = unsafe { L::pointers(node).as_ref().get_prev() } {
            unsafe {
                L::pointers(prev)
                    .as_mut()
            ...
            self.head = unsafe { L::pointers(node).as_ref().get_next() };
        }

        if let Some(next) = unsafe { L::pointers(node).as_ref().get_next() } {
            unsafe {
                L::pointers(next)
                    .as_mut()
            ...
            self.tail = unsafe { L::pointers(node).as_ref().get_prev() };
        }

        unsafe { L::pointers(node).as_mut().set_next(None) };
        unsafe { L::pointers(node).as_mut().set_prev(None) };

        Some(unsafe { L::from_raw(node) })
    }
}
...
mod tests {
    ...
    unsafe impl<'a> Link for &'a Entry {
        ...
        unsafe fn pointers(mut target: NonNull<Entry>) -> NonNull<Pointers<Entry>> {
            NonNull::from(unsafe { &mut target.as_mut().pointers })
        }
    }
