...
impl<T> AlignedBoxWithSlice<T> {
    ...
    pub fn new(capacity: usize, alignment: usize) -> AnnResult<Self> {
        if capacity == 0 {
            return Self::new_empty(alignment);
        }
    ...
    /// Creates an empty aligned allocation with no backing storage.
    pub fn new_empty(alignment: usize) -> AnnResult<Self> {
        let _ = alignment;
        Ok(Self {
            layout: Layout::new::<T>(),
            val: Vec::new().into_boxed_slice(),
        })
    }
    ...
    pub fn memcpy(&mut self, src: &[T]) -> AnnResult<()> {
        ...
        if src_ptr < dst_end && src_end > dst_ptr {
            return Err(AnnError::Index {
                err: "Source and destination overlap".to_string(),
            });
        }
