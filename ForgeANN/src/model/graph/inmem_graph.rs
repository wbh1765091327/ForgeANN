...
impl InMemoryGraph {
    ...
    pub fn read_vertex_and_neighbors(
        &'_ self,
        vertex_id: u32,
    ) -> Result<RwLockReadGuard<'_, VertexAndNeighbors>, AnnError> {
        Ok(self.final_graph[vertex_id as usize].read())
    }
    ...
    pub fn write_vertex_and_neighbors(
        &'_ self,
        vertex_id: u32,
    ) -> Result<RwLockWriteGuard<'_, VertexAndNeighbors>, AnnError> {
        Ok(self.final_graph[vertex_id as usize].write())
    }
}
...
mod graph_tests {
    ...
    fn test_new() {
        ...
        for i in 0..10 {
            let neighbor = graph.final_graph[i].read();
